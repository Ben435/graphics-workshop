...
void main() {
    ...
    float temperatureFactor = (-temperature / 100.0); // between -40 -> 40, so factor between -0.4 and 0.4
    vec3 mountainColor = vec3(0.12, 0.15, 0.1);
    mountainColor = mix(mountainColor, vec3(0.4, 0.32, 0.4), smoothstep(0.7, 0.8, mountainFactor));
    mountainColor = mix(mountainColor, vec3(0.9, 0.9, 0.9), smoothstep(1.0 - temperatureFactor, 1.5, mountainFactor));
